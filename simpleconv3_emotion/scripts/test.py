...
#一次测试一个文件
testimagepath = get_file_path(['data', 'testimages'])
imagepaths = os.listdir(testimagepath)
for imagepath in imagepaths:
    im = cv2.imread(os.path.join(testimagepath, imagepath), 1)
    try:
        rects = cascade.detectMultiScale(im, 1.3, 5)
        x, y, w, h = rects[0]
        rect = dlib.rectangle(x, y, x + w, y + h)
    ...
    im_show = cv2.imread(os.path.join(testimagepath, imagepath), 1)
    ...
    cv2.imwrite(get_file_path(["data", "results", imagepath]), im_show)
    cv2.waitKey(0)
